      ...
      {isOpen && (
        <>
          <motion.div 
            ...
            exit={{ opacity: 0 }}
            onClick={onCancel}
            className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[15000] touch-none"
          />
          <motion.div 
            initial={{ opacity: 0, scale: 0.95, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95, y: 20 }}
            className={`fixed left-1/2 -translate-x-1/2 bg-white rounded-[10px] p-6 z-[15010] shadow-2xl w-[90%] max-w-sm ${
              validationValue 
